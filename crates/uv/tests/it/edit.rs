...
async fn add_empty_ignore_error_codes() -> Result<()> {
    ...
    hint: An index URL (http://[LOCALHOST]/) returned a 403 Forbidden error. Check that the index URL is correct and the credentials are valid.
    hint: If you want to add the package regardless of the failed resolution, provide the `--frozen` flag to skip locking and syncing
    "
    );
    Ok(())
...
/// If a different package was read from an index before a 403, uv should suggest that the index
/// might use 403s for packages that are not found.
#[tokio::test]
async fn lock_forbidden_index_after_simple_api_success() -> Result<()> {
    let context = uv_test::test_context!("3.12");
